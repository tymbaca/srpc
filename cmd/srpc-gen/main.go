...
const clientTmpl = `// Code generated by srpc-gen {{ .Version }}. DO NOT EDIT.
...
func New{{ .Target }}Client(client *srpc.Client) *{{ .Target }}Client {
	return &{{ .Target }}Client{client: client}
}
...
func (c *{{ $.Target }}Client) {{ .Name }}(ctx context.Context, req {{ .ReqType }}) (resp {{ .RespType }}, err error) {
	err = c.client.Call(ctx, "{{ $.Target }}.{{ .Name }}", req, &resp)
	return resp, err
}
{{- end }}
`

const serverTmpl = `// Code generated by srpc-gen {{ .Version }}. Edit for your needs.
...
type {{ .Target }}Server struct {
	// TODO: fill
}

{{- range .Methods }}

func (s *{{ $.Target }}Server) {{ .Name }}(ctx context.Context, req {{ .ReqType }}) ({{ .RespType }}, error) {
